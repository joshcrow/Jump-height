...
| **WOO mount** (WOO Sports 65240-7203, ~€15) — **ORDERED 2026-09-15** | The kite world's field-proven board mount: a self-adhesive (3M) cradle, sensor held by two rubber O-rings, long side parallel to the board, tool-free on/off. **WOO 4.0 sensor body: 5.4 × 2.2 × 1.7 cm, sensor + mount "about as much as an AA battery"** ([easy-surfshop, WOO 4.0 kit page](https://easy-surfshop.com/do/item/WOO-65240-7200/Kiteboard-sensor-WOO-4.0--charger-and-mount)); the mount itself is still unstated — and the page's own sentence order is *"The sensor and mount together weigh about as much as an AA battery. Dimensions: 5.4 x 2.2 x 1.7 cm."*, so whether 54 × 22 × 17 is the sensor BODY or sensor-plus-cradle is **ambiguous on the only source** (caught by the enclosure design review, 2026-09-15); if it is the pair, the body is smaller and every 22 mm-wide design is the wrong size. The mount in hand settles it in one caliper pass. Read on with that caveat: the cradle is cut for a **22 mm-wide** body and the Hammond case is **35 mm** wide, 13 mm too much — the as-is fit is **unlikely** on paper; measure anyway when it arrives. A WOO-shaped shell is plausible for the electronics: the Sense is 21 × 17.8 mm, i.e. 2 mm walls at 22 mm; the installed cell is an **LP502030 + PCM** (docs/battery-measurement.md, identified 2026-08-18): the code says **5.0 × 20 × 30 mm** nominal, plus the PCM tab — caliper the real one, it decides the thickness. **When it arrives: measure the cradle and the O-ring span, then try the Hammond 1551WHGY (60 × 35 × 22 mm) in it.** Holds → it replaces the two GoPro bases below. Doesn't → those numbers are the spec for the next enclosure, and matching the WOO footprint means any WOO owner can drop our puck into the mount already on their board. It has **no tether anchor** — DECISION #9 (float + tethered) still needs a leash to a footstrap insert. **The rider's board is a 2025 F-One Rocket Wing-S V4** (rider's own correction, 2026-09-15; blue with black accent): the adhesive wants a smooth surface, so the mount goes on the **flat section forward of the deck pad** (owner, 2026-09-15). F-One describes the Rocket Wing-S deck as a full pad with a **"domed front deck"** ([f-one.world](https://www.f-one.world/product/rocket-wing-s-2/)) — a rigid ~54 mm plate on a dome contacts at the crown only, so lay a straight edge across the intended spot and check the gap before sticking; on curvature use a thicker VHB pad or shim, alcohol-wiped, 24 h before water. |
| **ENCLOSURE DECISION 2026-09-15 (evening)** — see `/Users/joshcrow/Jump-height-race-openai/hardware/enclosure/HANDOFF-2026-09-15.md` (branch `enclosure-race-openai`, c823de6) | Keep the Sense, the tested EEMB 250 mAh cell (listing 32 × 20.5 × 5.3 mm) and its soldered pigtail. **Build in the current Hammond 1551WHGY (60 × 35 × 22 mm incl. lid).** USB: a factory **CA-USBW1** panel-mount USB-C extension (two ordered) plugs into the Sense's native socket inside and gives an external socket with a screw cap on the broad lid — no desoldering, no custom PCB; lid stays closed for charging and data, cap off only on dry land. Flotation now optional; independent tether stays. The WOO holder (ordered) may be modified; try the case flat and on its long edge (60 × 22 × 35 on edge — same width as the WOO body, 6 mm longer, 18 mm taller). **Every custom-connector / carrier-PCB design (hardware/enclosure/woo-fit, the sibling woo-fit-openai) is superseded** and kept as a study. Nothing is verified yet: no USB fit, electrical, leak or retention test has run. The WOO body 54 × 22 × 17 is confirmed as the SENSOR body by the WOO manual's technical table (handoff, size-ground-truth), which retires the sensor-vs-mount ambiguity noted above. |
