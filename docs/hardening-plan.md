...
| item | size | proof |
|---|---|---|
| **Calibration provenance selftest row** (R1): an automated FAIL when **any single** calibration key fell back to defaults. *(Premise corrected by review: the mechanism half-exists — `loadCalibration()` reads the flags and `info` prints `source=`; the OG's loss WAS surfaced there and nobody was looking. The real gaps: it is manual, and the OR across keys hides single-key fallbacks.)* | S | wipe ONE key on the spare while others survive; row FAILS; restore; PASSES |
