...
| **F-31** | partial | **The suite did not read its own constants.** A 333-mutant overnight campaign found five gap classes, all the same shape: deliberately-chosen constants that nothing asserted. Worst two — `regression_check`'s failure paths were all flippable to pass, and `lever_arm`'s measured-wrong 5 % shave could be reinstated silently. Suite 249 → 319; no behaviour changed. **PARTIAL:** 7 of 11 modules triaged; `wing_model`, `seastate`, `windows`, `generate` (130 mutants) were still running when this was filed — the first two are the physics behind E1–E16. `selfdiag` killed 11/11, a clean sweep and F-26's fix holding |
