...
| What is on the connected USB bench board? | **2026-09-15: `JumpHeight-8673`, UID `2513620E30AE413D`, runs research USB recorder `9c9289b69af5b76a`; it no longer speaks the product protocol.** USB-only, no BLE or product detector. The clock study is blocking calibration/motion tests. Instrumented build `53aac69c2b7e37a8` is prepared but **not installed**: software DFU entry returned to the old app; no upload occurred. | `research/usb_recorder/captures/clock-study-20260915/deployment/deployment-status.json`; [clock study](../research/usb_recorder/TIMEBASE_STUDY.md); [exact rollback ZIP, SHA-256 and restoration procedure](bench-playbook.md#research-recorder-lookup-and-restoration--2026-09-15) |
...
| **Nick's board / mount** | **2025 F-One Rocket Wing-S V4**, blue with black accent (rider's own correction via the owner, 2026-09-15): a flat un-padded section forward of the deck pad is the mount site. F-One calls that front deck **domed**; whether the WOO plate seats flat there is unmeasured (straight-edge check first, BUILD.md). Puck has only ever ridden in a **vest pocket**. A **WOO mount** (self-adhesive cradle + O-rings) was ordered 2026-09-15 for the nose flat. **Decision, evening 2026-09-15: keep the Hammond 1551WHGY and add a factory CA-USBW1 capped USB-C extension on the lid; the WOO-fit custom-connector designs are superseded** (BUILD.md, the enclosure row; handoff on branch `enclosure-race-openai`). Fit in the cradle, flat or on edge, is **unmeasured** until the mount arrives. | this session, 2026-09-15 |
