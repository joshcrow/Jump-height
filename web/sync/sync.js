...
function setEnabled() {
  ...
  // `transport` is part of the gate, not just of doClear(): you cannot empty a
  // puck you are not connected to, and after a lost link the button must not
  // be on screen at all — the one-button rule, measured broken here on
  // 2026-09-12 (btn-connect-usb and btn-clear both visible after a cable drop).
  const offerClear = !!(transport && S && S.verified && !S.cleared
...
function setVisible(offerClear) {
  ...
  $('send-hint').hidden = !((S && S.saveCount) >= 2);
